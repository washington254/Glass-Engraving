...
export function BottomLogoUI({ onLogoUpload, onTextChange, currentText = '', cameraControls }) {
  ...
  return (
    <div
      ...
      className={`absolute bottom-4 left-4 right-4 md:top-4 md:left-4 md:bottom-auto md:w-80 w-auto max-h-[100dvh] md:max-h-[94dvh] overflow-hidden ${isCollapsed ? 'bg-primary-700' : 'bg-gray-900/90'} backdrop-blur-sm rounded-lg shadow-xl border border-gray-700 transition-all duration-500 ease-in-out ${isCollapsed ? 'p-4' : 'p-6'} cursor-pointer md:cursor-default ${visibilityClass}`}
    >
