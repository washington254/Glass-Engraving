...
export function StickerUI({ onImageUpload, onTextAdd, cameraControls }) {
  ...
  return (
    <div
      ...
      className={`absolute top-10 mt-7 md:top-4 md:mt-0 left-4 right-4 md:left-auto md:right-4 md:w-80 md:translate-x-0 w-auto max-h-[100dvh] md:max-h-[94dvh] overflow-hidden  ${isCollapsed ? 'bg-primary-700' : 'bg-gray-900/90'} backdrop-blur-sm ${isCollapsed ? 'rounded-2xl' : 'rounded-lg'} shadow-xl border border-gray-700 transition-all duration-500 ease-in-out  cursor-pointer md:cursor-default ${visibilityClass}`}
    >
              ...
              <div className="grid grid-cols-4 gap-2">
                {['1A.webp', '2A.webp', '3A.webp', 'logo.webp'].map((preset, index, array) => (
                  <button
                    key={preset}
                    onClick={() => {
                      onImageUpload(`/${preset}`, 'image/webp');
